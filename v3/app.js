...
/* Direction 03 — visual exploration.
   Search and readout run on the real dataset and the real coverage
   math (see ../README.md) rather than mocked copy. */
...
function renderQuiver() {
  ...
  quiver.forEach((ski) => {
    ...
    btn.innerHTML = `<svg viewBox="0 0 12 12" width="8" height="8" fill="none" aria-hidden="true"><path d="M1.5 1.5 L10.5 10.5 M10.5 1.5 L1.5 10.5" stroke="currentColor" stroke-width="1.9" stroke-linecap="round"/></svg>`;
...
function renderReadout() {
  if (quiver.length === 0) {
    readoutEl.textContent = "Nine zones of ski day. Add a ski to see which ones you cover.";
    return;
  }
  ...
  readoutEl.innerHTML = missing.length === 0
    ...
    : `${9 - missing.length} of 9 zones covered — <span class="gap">biggest hole: ${missing[0].label}</span>`;
}
