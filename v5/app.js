...
/* Direction 02 — visual exploration.
...
const countEl = document.getElementById("readout-count");
...
function renderReadout() {
  if (quiver.length === 0) {
    countEl.textContent = "0 / 9";
    noteEl.textContent = "Add a ski to read your gaps.";
    return;
  }
  const z = zones(quiver);
  const missing = z.filter((x) => !x.covered);
  countEl.textContent = `${9 - missing.length} / 9`;
  noteEl.textContent = missing.length === 0
    ? "Every zone has a ski built for it."
    : `Biggest hole: ${missing[0].label}.` + (missing.length > 1 ? ` ${missing.length - 1} more uncovered.` : "");
}
